...
{if ! $rsUserOrders}
  Нет заказов
{else}
  <table border="1" cellspacing="1" cellpadding="1" bordercolor="#5F9EA0">
    <tr>
      <th>№</th>
      <th>Действие</th>
      <th>ID заказа</th>
    ...
    {foreach $rsUserOrders as $item name=orders}
      <tr>
        ...
        <td><a href="#" onclick="showProducts('{$item['id']}'); return false;">Показать товар заказа</a>
        <td>{$item['id']}</td>
        <td>{if $item['status']==0} Заказ не оплачен
            {else} Заказ оплачен
            {/if}</td>
